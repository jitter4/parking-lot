package demo.design.level.low.parkinglot.services.authentication.filters;

import demo.design.level.low.parkinglot.services.authentication.configurations.security.UserPrincipal;
...
public class JWTAuthenticationFilter extends OncePerRequestFilter {
    ...
    public JWTAuthenticationFilter(JwtUtils jwtUtils, SecurityProperties securityProperties) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (StringUtils.isBlank(utid)) {
            sendErrorResponse(401, this.securityProperties.getFailureMessage(), response);
            return;
        }
        ...
        if (!this.jwtUtils.validateExpirationdate(claims)) {
            sendErrorResponse(HttpStatus.UNAUTHORIZED.value(), this.securityProperties.getFailureMessage(), response);
            return;
        }
        ...
        if (StringUtils.isNotBlank(remoteAddr)) {
            if (!StringUtils.equals(remoteAddr, request.getRemoteAddr())) {
                sendErrorResponse(HttpStatus.FORBIDDEN.value(), this.securityProperties.getFailureMessage(), response);
                return;
            }
        }
